...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    _visualisationThread = new ThreadVisualisation(this, ui.render_widget->renderWindow());
    ...
    if (arguments.size() > 1)
        for (int argCount = 1; argCount < arguments.size(); argCount++) {
            ...
            if (argument.compare("help") == 0) {
                Log::Info(
                    "Usage: jpsvis [file1] [-2D] [-caption] [-online [port]]");
                exit(0);
...
void MainWindow::parseGeometry(const QDomNode& geoNode) {
    ...
    QString fileName = geoNode.toElement()
    ...
    // which is the only one which can directly be inserted into a file
        ...
        QDomNode geoNode;
        if (!geoNode.isNull()) {
            Log::Error("online geo: %s", geoNode.toElement()
                                             .toDocument()
                                             .toString()
                                             .toStdString()
                                             .c_str());

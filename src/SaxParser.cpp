...
void SaxParser::parseGeometryXMLV04(QString filename, GeometryFactory & geoFac)
{
    ...
    auto geo = std::shared_ptr<FacilityGeometry>(new FacilityGeometry("no name", "no name", "no name"));
    ...
    for(int i = 0; i < xSubRoomsNodeList.length(); i++) {
        ...
        std::string subroomCaption = xSubRoomsNodeList.item(i).toElement().attribute("id").toStdString();
